local State_041SelectSongMultipleCharts = {}

-- INPUT HANDLING

function State_041SelectSongMultipleCharts.UpPressed()
...
function State_041SelectSongMultipleCharts.DownPressed()
...
function State_041SelectSongMultipleCharts.CenterPressed()
...
function State_041SelectSongMultipleCharts.BackPressed()
...
function State_041SelectSongMultipleCharts.Drawing()
	local drawingX = 6
	...
	for j=1,#Game.selectedSongArrayOfCharts,1 do
		...
		meckx_print({
			...
			XPos = drawingX+24,
			YPos = drawingY,
			ColorName = FetchChartColorEnabled(songToInput, chartToInput),
			FontStyle = ClassicConsole_48,
		})
		drawingY = drawingY + linebreakSize
	end
	...
	for j=1,#Game.selectedSongArrayOfCharts,1 do
		...
		local actualText
		local actualColor
		local recommendedSpeed = FetchRecommendedSpeed(Game.selectedPlayerName, songToInput, chartToInput)
		local highScoreObject = FetchHighScore(playerPlaying, songToInput, chartToInput)
		if highScoreObject then
			if recommendedSpeed then
				actualText = "[" .. recommendedSpeed .. "] " .. highScoreObject.DisplayAccuracy
			else
				actualText = highScoreObject.DisplayAccuracy
			end
			actualColor = FetchHighScoreColor(playerPlaying, songToInput, chartToInput)
		else
			if recommendedSpeed then
				actualText = "[" .. recommendedSpeed .. "] " .. "-"
			else
				actualText = "-"
			end
...
return State_041SelectSongMultipleCharts